...
getpower <- function(m=1000, N=72, t.points=c(0,1,2,3,4), 
  ...
  if(hyp == "both" | hyp == "b"){
    ...
    # return power for H0 only
    return(list(power.H0 = length(bfs[bfs>BFthres])/m))
    
  } else if(hyp == "h1" | hyp == "1"){
    
    # return power for H1 only
    return(list(power.H1 = length(bfs[bfs>BFthres])/m))
  }
}
