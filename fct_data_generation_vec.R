...
dat.gen.vec <- function(gen.H0=T, m=100, N=72, t.points=c(1,2,3,4,5), var.u0=0, var.u1=.1, var.e=.02, eff.size=.8, BFthres=3){
  ...
  dat.H1 <- lapply(dat.H1.empty, make.y)
  ...
  results.H1 <- lapply(1:m, function(i) {
    result.H1 <- bain(x = est.H1[i], Sigma = list(sig.H1[[i]]), hypothesis = hypotheses, n=N*n, group_parameters = 1, joint_parameters = 0)
