...
dat.gen <- function(m=100, N=100, t.points=c(1,2,3,4,5), var.u0=.0333, var.u1=.1, var.e=.0262, eff.size=.8, BFthres=3){
  ...
  for (i in 1:m) {
    ...
    result.H0 <- bain(est.H0, hypotheses <- "t:treat>0;t:treat=0", n = N*n, Sigma = sig.H0, group_parameters = 1, joint_parameters = 0)
    ...
    result.H1 <- bain(est.H1, hypotheses <- "t:treat>0;t:treat=0", n = N*n, Sigma = sig.H1, group_parameters = 1, joint_parameters = 0) 
    BFs.H1[[i]] <- result.H1$BFmatrix[1,2]
  }
  return(output = list(Median_BF0 = median(unlist(BFs.H0)), Median_BF1 = median(unlist(BFs.H1)), Prop_BF0 = length(BFs.H0[BFs.H0>BFthres])/m, 
