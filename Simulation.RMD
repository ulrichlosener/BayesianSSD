...
bayesian.SSD <- function(true.hyp = 1, n.steps = 20, m = 1000, eff.size = .8, BFthresh = 3, t.points = c(0,1,2,3,4)) {
  ...
  for(j in 1:n.steps){
    ...
    t <- rep(t.points, N)
    id <- rep.int(seq_len(N), each=n)
    treat <- as.numeric(as.character(gl(n=2, k=n, length=N*n, labels=c(0,1))))
    dat0 <- data.frame(id, treat, t)
    
    beta0 <- rep.int(0, N*n) # average y at t0 
    beta1 <- rep.int(0, N*n) # average increase for x=0
    beta2 <- eff.size * sqrt(sigmasq.u1) * rep(true.hyp==1, N*n) # average difference in slopes between conditions
    ...
    prop.bf10[j] <- length(bf10[bf10>BFthresh])/m
    prop.bf01[j] <- length(bf01[bf01>BFthresh])/m
