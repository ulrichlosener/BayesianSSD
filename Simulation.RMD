...
#for(u in 3:10){
  ...
  sigmasq.u1 <- .01   # variance of individual deviation from treatment slope
...
```{r}
  ...
  ggplot(data=df.prop.bf10.H1, aes(x=N, y=prop.bf)) + 
    geom_line() +
    geom_hline(yintercept=eta, col="red", linetype="dashed")
  
  bf10.H1.trim <- bf10.H1[bf10.H1<=quantile(bf10.H1, probs = .85)]
  
  plot(density(bf10.H1.trim), xlim=range(0:200))
  polygon(c(density(bf10.H1.trim)$x[density(bf10.H1.trim)$x >= 3 ], 3),
        c(density(bf10.H1.trim)$y[density(bf10.H1.trim)$x >= 3 ], 0),
        col = "slateblue1",
        border = 1)
  
p <- ggplot() + geom_density(aes(x=bf10.H1.trim), size=1) + 
  xlim(0, 150) + 
  geom_vline(xintercept=3, linetype=2, color="red") +
  annotate("text", x=0, y=.02, label="BFthresh=3", angle=90) +
  labs(title = TeX(r"(Sampling Distribution of $BF_{10}$ under $H_1$ for $N=176$)"), y="density", x=TeX("$BF_{10}$"))
