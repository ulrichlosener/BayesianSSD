...
models <- vector("list", n.steps)

for(i in 1:n.steps){
  models[[i]] <- vector("list", m)
}
...
for(j in 1:n.steps){
  ...
  beta2 <- ifelse(true.hyp==1, rep(.5, N*n), rep(0, N*n)) # average difference in slopes between conditions
  
  #eff.size <- beta2/sqrt(sigmasq.u1)
  
  bf10 <- numeric(m) # storage for BFs
  bf01 <- numeric(m) # storage for BFs
  bf.u <- numeric(m) # storage for BFs
  bf.c <- numeric(m) # storage for BFs
  pmp.a <- numeric(m) # storage for PMPs
  pmp.b <- numeric(m) # storage for PMPs
  pmp.c <- numeric(m) # storage for PMPs
  ...
  for (i in 1:m) {
    ...
    models[[j]][[i]] <- lmer(y ~ t + t:treat + (t | id), data = dat)
    if (isSingular(models[[j]][[i]])) {next}
  ...
  medbf10[j] <- median(bf10)
  quantbf10[j] <- quantile(bf10, probs=0.2)
  prop.bf10[j] <- length(bf10[bf10>3])/m
  prop.bf01[j] <- length(bf01[bf01>3])/m
  medbf01[j] <- median(bf01)
...
# marker <- rep(NA, m)
# for(i in 1:m){
#   marker[i] <- ifelse(isSingular(models[[20]][[i]]), 1, 0)
# }
...
```{r}
av.est <- rep(NA, n.steps)
for(i in 1:n.steps){
av.est[i] <- models[[i]][[1]]@beta[3]
}
```
