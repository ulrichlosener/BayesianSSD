...
for(j in 1:n.steps){
  ...
  beta2 <- ifelse(true.hyp==1, rep(.8, N*n), rep(0, N*n)) # average difference in slopes between conditions
  ...
  for (i in 1:m) {
    ...
    inter <- lme(y ~ t + t:treat, random =~ t | id, data = dat, control = lmeControl(opt="optim", msMaxIter=1000, msMaxEval=1000))
...
ss.seq <- seq(from=25, to=N, by=5)
beta.seq <- seq(.1, 1, .1)

# par(mfrow=c(2,2))
# plot(x=ss.seq, y=mbf12[1:length(ss.seq)], type="l", xlab="N", ylab="mean BF12")
# plot(x=ss.seq, y=mbf21[1:length(ss.seq)], type="l", xlab="N", ylab="mean BF21")
...
plot(x=ss.seq, y=prop.bf12[1:length(ss.seq)], type="l", xlab="N", ylab="proportion of BFs larger 3")
abline(h=.8, col="red")
